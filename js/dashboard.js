 ...
 * Handles dynamic color statuses, breakdown tiers, single-click table filtering, 
 * and double-click routing to reports.
 */
const DashboardController = {
  ...
  showToast(message, type = "success") {
    ...
    if (toastEl && msgEl) {
      toastEl.className = "toast align-items-center text-white border-0 shadow bg-" + (
        type === "error" ? "danger" : type === "warning" ? "warning text-dark" : "dark"
      );
  ...
  /**
   * Centralized Status & Color Metadata Engine
   * Separates 16-30d (Green) from 31-45d (Cyan)
   */
  getStatusMeta(days) {
    if (days === null || days === undefined) {
      return {
        textClass: "text-muted",
        pillClass: "badge-notice",
        label: "No Date",
        icon: "⚪"
      };
    }
    if (days <= 0) {
      return {
        ...
        pillClass: "badge-expired",
        label: days === 0 ? "Today" : Math.abs(days) + "d Over",
        icon: "🚨"
      };
    }
    if (days <= 15) {
    ...
    // 16-30 Days: Emerald Green
    if (days <= 30) {
      return {
        textClass: "text-status-safe",
        pillClass: "badge-safe",
        label: days + "d Left",
        icon: "🟢"
      };
    }
    // 31-45 Days: Cyan / Ocean Blue (Notice Tier)
    return {
      textClass: "text-status-notice",
      pillClass: "badge-notice",
      label: days + "d Left",
      icon: "📢"
    };
  ...
  generateCustomerWhatsAppLink(customer, clientVehicles) {
    ...
    const urgentVehicles = clientVehicles.filter(
      v => v.min_days_remaining !== null && v.min_days_remaining <= 30
    );
  ...
  renderActivityTicker() {
    ...
    const activities = [
      "[" + nowStr + "] Verified " + this.vehicles.length + " vehicle compliance records across " + this.customers.length + " clients."
    ];
    ...
    tickerBox.innerHTML = activities
      .map(act => '<div class="activity-item"><i class="bi bi-dot text-primary"></i><span>' + this.escape(act) + '</span></div>')
      .join("");
  },

  renderDocumentBreakdowns() {
    const counts = {
      fc: { expired: 0, warning: 0, expiring: 0, notice: 0 },
      permit: { expired: 0, warning: 0, expiring: 0, notice: 0 },
      np: { expired: 0, warning: 0, expiring: 0, notice: 0 },
    ...
    const check = (key, dateStr, isApplicable = true) => {
      ...
      else if (days <= 45 && counts[key].notice !== undefined) counts[key].notice++;
    };
    ...
    this.safeSetText("docFcNotice", counts.fc.notice);

    this.safeSetText("docPermitExpired", counts.permit.expired);
    this.safeSetText("docPermitWarning", counts.permit.warning);
    this.safeSetText("docPermitSafe", counts.permit.expiring);
    this.safeSetText("docPermitNotice", counts.permit.notice);

    this.safeSetText("docNpExpired", counts.np.expired);
    this.safeSetText("docNpWarning", counts.np.warning);
    this.safeSetText("docNpSafe", counts.np.expiring);
    this.safeSetText("docNpNotice", counts.np.notice);
  ...
  filterTracker(docKey, tier, btnEl) {
    if (this.activeTrackerFilters[docKey] === tier) {
      ...
      btnEl.classList.remove("active-filter");
    } else {
      this.activeTrackerFilters[docKey] = tier;
      const cardEl = btnEl.closest(".dash-card");
      if (cardEl) {
        cardEl.querySelectorAll(".btn-tier-filter").forEach(b => b.classList.remove("active-filter"));
  ...
  handleTrackerBtnDblClick(docKey, tier) {
    const presetMap = {
      "0d": "EXPIRED_ONLY",
      "15d": "FINE_ONLY",
      "30d": "SAFE_ONLY",
    ...
    const preset = presetMap[tier] || "ALL";
    window.location.href = "reports.html?doc=" + docKey + "&preset=" + preset;
  },
  ...
  renderTrackerRows(list, fieldName, fieldLabel, docParam) {
    return list.slice(0, 5).map(item => {
      const meta = this.getStatusMeta(item.days);
      const rawDate = item.vehicle[fieldName];
      const displayDate = this.formatDisplayDate(rawDate);

      return (
        '<tr ondblclick="window.location.href=\'reports.html?vehicle_id=' + item.vehicle.vehicle_id + '&doc=' + docParam + '\'" title="Double-click to view Report" style="cursor: pointer;">' +
          '<td class="fw-bold font-monospace">' +
            '<span class="reg-plate-link">' + this.escape(item.vehicle.registration_no) + '</span>' +
          '</td>' +
          '<td class="text-center font-monospace ' + meta.textClass + '" ' +
              'ondblclick="event.stopPropagation(); DashboardController.openQuickEditModal(\'vehicle_date\', \'' + item.vehicle.vehicle_id + '\', \'' + fieldName + '\', \'' + fieldLabel + '\', \'' + rawDate + '\')" ' +
              'title="Double-click to edit Date">' +
            displayDate +
          '</td>' +
          '<td class="text-end">' +
            '<span class="doc-badge-link ' + meta.pillClass + '">' +
              meta.label +
            '</span>' +
          '</td>' +
        '</tr>'
  ...
  renderFcTracker() {
    ...
    if (list.length === 0) {
      tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted py-3">No vehicles in this tier.</td></tr>';
      return;
    }
    tbody.innerHTML = this.renderTrackerRows(list, "fc_expiry", "FC Expiry", "fc");
  },
  ...
  renderPermitTracker() {
    ...
    tbody.innerHTML = this.renderTrackerRows(list, "permit_expiry", "Permit Expiry", "permit");
  },
  ...
  renderNpTracker() {
    ...
    tbody.innerHTML = this.renderTrackerRows(list, "national_permit_expiry", "NP Expiry", "np");
  },
  ...
  renderPriorityQueue() {
    ...
    const priorityItems = this.vehicles
      ...
      .map(v => {
        ...
        return Object.assign({}, v, { priorityTier: tier });
      });
    ...
    let visibleList = [].concat(priorityItems);
    if (this.priorityQueueFilter !== "ALL") {
      visibleList = visibleList.filter(v => v.priorityTier === this.priorityQueueFilter);
    }

    visibleList.sort((a, b) => ((a.min_days_remaining !== null ? a.min_days_remaining : 999) - (b.min_days_remaining !== null ? b.min_days_remaining : 999)));
    ...
    tbody.innerHTML = visibleList.slice(0, 10).map(v => {
      ...
      const days = v.min_days_remaining;
      const meta = this.getStatusMeta(days);
      ...
      return (
        '<tr>' +
          ...
          '<td class="text-center font-monospace fw-bold ' + meta.textClass + '">' +
            this.formatDisplayDate(criticalDate) +
          '</td>' +
          '<td class="text-end"><span class="doc-badge-link ' + meta.pillClass + '">' + meta.label + '</span></td>' +
          '<td class="text-end">' +
            (waLink ? '<a href="' + waLink + '" target="_blank" class="btn-whatsapp-icon" title="Send WhatsApp"><i class="bi bi-whatsapp whatsapp-icon"></i></a>' : "-") +
          '</td>' +
        '</tr>'
      );
    }).join("");
